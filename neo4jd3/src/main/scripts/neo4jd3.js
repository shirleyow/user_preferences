    ...
    function neo4jDataToD3Data(data) {
        ...
        data.results.forEach(function(result) {
            result.data.forEach(function(data) {
                ...
                data.graph.rels.forEach(function(relationship) {
                    relationship.source = relationship.startNode;
                    relationship.target = relationship.endNode;
                    graph.relationships.push(relationship);
                });

                data.graph.rels.sort(function(a, b) {
                ...
                for (var i = 0; i < data.graph.rels.length; i++) {
                    if (i !== 0 && data.graph.rels[i].source === data.graph.rels[i-1].source && data.graph.rels[i].target === data.graph.rels[i-1].target) {
                        data.graph.rels[i].linknum = data.graph.rels[i - 1].linknum + 1;
                    } else {
                        data.graph.rels[i].linknum = 1;
                    }
                }
            });
        });
