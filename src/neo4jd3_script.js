  ...
  try {
    switch (search_mtd) {
      case "0":
        ...
        await session2
          .run(
            "MATCH (u:User{UserID:$userid})-[:READ]->(d:Document{DocID: $inp})-[r]->(n) WHERE NOT 'User' in labels(n) WITH [d]+COLLECT(n) as a, COLLECT(r) as b CALL apoc.export.json.data(a, b, '../../../../../../xampp/htdocs/user_preferences/assets/user_data.json', {jsonFormat:'JSON'}) YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data",
            { userid: userid, inp: inp }
          ) // By default exports to neo4j relate-data dmbs
          .catch((error) => {
            console.log(error);
          });
        break;
    ...
    await $.getJSON("../assets/user_data.json", function (json) {
      json.nodes.forEach((obj) => {
        delete obj.properties["calc_score"];
        if (obj.labels[0] == "Document") {
          obj.properties["Text"] = truncateString(
            obj.properties["text"].replace(/\\/g, ""),
            400
          );
        ...
        if (obj.labels[0] == "Topic") {
          ...
          [...Array(words_len).keys()].forEach((i) => {
            word_function +=
              Number(obj.properties.word_weights[i].toPrecision(3)) +
              "*" +
              obj.properties.words[i]; // construct word function
...
async function expandDocNode(docid) {
  ...
  // This function still works, currently, just avoiding the issue by disallowing double clicking when doc nodes are expanded and graph is resetted.
  // Refreshing the page would also 'resolve' the issue.
  expandedDoc = true;
  try {
    await session2
      .run(
        "MATCH (d:Document{DocID: $docid})-[r]->(n) WHERE NOT 'User' in labels(n) WITH COLLECT(n) as a, COLLECT(r) as b CALL apoc.export.json.data(a, b, '../../../../../../xampp/htdocs/user_preferences/assets/doc_data.json', {jsonFormat:'JSON'}) YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data",
        { docid: docid }
      )
      .catch((error) => {
        console.log(error);
...
const init = async function () {
  try {
    await session2
      .run(
        "MATCH (u:User{UserID:$userid})-[r:READ]->(d:Document) WITH COLLECT(distinct(u))+COLLECT(d) as a, COLLECT(r) as b CALL apoc.export.json.data(a, b, '../../../../../../xampp/htdocs/user_preferences/assets/user_data.json', {jsonFormat:'JSON'}) YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data",
        { userid: userid }
      ) // By default exports to neo4j relate-data dmbs
      .catch((error) => {
        console.log(error);
    ...
    await $.getJSON("../assets/user_data.json", function (json) {
      json.nodes.forEach((obj) => {
        ...
        if (obj.labels[0] == "User") {
          ...
          if (obj.properties["disliked_tops"].length != 0)
            obj.properties["Disliked Topics"] = obj.properties["disliked_tops"];
          if (obj.properties["disliked_ents"].length != 0)
            obj.properties["Disliked Entities"] =
              obj.properties["disliked_ents"];
          if (obj.properties["liked_tops"].length != 0)
            obj.properties["Liked Topics"] = obj.properties["liked_tops"];
          if (obj.properties["liked_ents"].length != 0)
            obj.properties["Liked Entities"] = obj.properties["liked_ents"];
